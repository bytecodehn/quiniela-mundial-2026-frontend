...
param(
    ...
    [int]   $HostPort = 8090,
    [string]$Container = "quiniela_frontend",
    [switch]$Push
)
...
Write-Host "[deploy] Ejecutando scripts/deploy.sh (branch=$Branch, port=$HostPort, container=$Container) en el servidor..." -ForegroundColor Cyan
Invoke-Remote "BRANCH=$Branch HOST_PORT=$HostPort CONTAINER=$Container bash $RepoDir/scripts/deploy.sh"
